...
def score_and_analyze(df, model_name, conversation_input_type, aggregation_target, target_scenario='inaccessible'):
    ...
    tiled_answerability_list_qas['binarized_model_answer'] = tiled_answerability_list_qas.apply(lambda x: x['target_character'].lower() in x['prediction'].lower() if pd.notnull(x['prediction']) else False, axis=1)
    ...
    tiled_accessibility_list_qas['binarized_model_answer'] = tiled_accessibility_list_qas.apply(lambda x: x['target_character'].lower() in x['prediction'].lower() if pd.notnull(x['prediction']) else False, axis=1)
...
def evaluate_bigtom_general_rules(inputs, model_responses, output_file=None):
    ...
    stats = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: {'correct': 0, 'total': 0}))))    
    # Iterate over all 8 rules
    for rule_index in range(8):
        ...
        for idx, item in enumerate(inputs):
            ...
            input_story_index = meta.get('story_index')
            input_within_story_index = meta.get('within_story_index') 
            ...
            model_response = model_responses[input_story_index][input_within_story_index][rule_index].lower().strip()

            stats[rule_index][cond][var][ib]['total'] += 1
            correct = true_answer_key in model_response 
            if correct:
                stats[rule_index][cond][var][ib]['correct'] += 1
